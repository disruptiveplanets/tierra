    ...
    for i in range(NDim):
        for j in range(NDim):
            if j<i:
                ...
                if Values:
                    ax[i,j].plot(Values[i],Values[j], "r+", markersize=30)
    ...
    for i in range(NDim):
        for j in range(NDim):
            if j<i:
                ...
                if Values:
                    ax[i,j].plot(Values[j], Values[i], "k+", lw=2, markersize=50)
                ...
                if Median<500 and Median>0.001:
                    MedianStr = "%0.2f" %Median
                else:
                    MedianStr = "%0.2e" %Median
                ...
                Title = Parameters[i]+ " = %s$^{+%s}_{-%s}$" %(MedianStr, UpperErrorStr, LowerErrorStr)
                print(Title)
                ax[i,j].set_title(Title, loc='left')#, pad=130)#, rotation=30)
